...
const fetchData = async () => {
  ...
  console.log(
    "ownerId,agentVersion,createdAt,workspaceHandle,plugin,billingCredits"
  );
  agents.forEach((agent) => {
    const agentCosts = costAggregations.filter(
      (x: { workspaceHandle: string | null }) =>
        x.workspaceHandle === agent.workspaceHandle
    );
    agentCosts.forEach((cost: { resourceHandle: any; billingCredits: any }) => {
      if (cost.billingCredits != 0) {
        console.log(
          `${agent.ownerId},${
            agent.agentVersion
          },${agent.createdAt.toISOString()},${agent.workspaceHandle},${
            cost.resourceHandle
          },${cost.billingCredits}`
        );
      }
